...
async function checkHealth() {
  try {
    const response = await fetchWithTimeout(`${API_BASE}/health`, {method: "GET", cache: "no-store"}, 8000);
...
async function sendQuestion(question) {
  ...
  try {
    if (!state.online && !(await checkHealth())) {
    ...
    const meta = typing.querySelector(".message-meta");
    if (meta) meta.textContent = error?.name === "AbortError" ? "TIMEOUT" : "OFFLINE";
    setStatus("OFFLINE");
    console.error(error);
  } finally {
    state.busy = false;
